...
Все разделы перевёрстаны по трём согласованным ImageGen-референсам из `output/fionin-tattoo/full-page-refs` корневого репозитория. Их фотографии находятся в `dist/assets/editorial/sheet-1.png`–`sheet-3.png`: CSS кадрирует только фотографические области, интерфейс и тексты реализованы в HTML. Сгенерированные портреты и интерьер обозначены как иллюстрации. Портфолио и отзывы используют проверенные материалы студии.

Галерея показывает три работы и раскрывает остальные по кнопке. Повторно проверены фильтры, просмотр фото, мобильное меню и подготовка сообщения; горизонтальной прокрутки нет на ширинах 320, 390, 768 и 1440 пикселей.
